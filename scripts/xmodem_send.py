#!/usr/bin/env python3
#
# Vendored from Seeed-Studio/sscma-example-we2 (MIT, Himax Technologies).
# Local modifications documented in memory/reference_xmodem_send_vendored_mods.md.
#
import serial
import xmodem
import time
...
if __name__ == '__main__':
    ...
    # Auto-confirm the WE-2 bootloader's "reboot? (y)" prompt, then exit on
    # the WE-2 application's INIT@STAT? end-of-boot marker. CH343 stays
    # enumerated through the reboot so we cannot rely on serial disconnect.
    POST_Y_IDLE_S = 6.0
    y_sent = False
    y_sent_at = None
    ...
    while True:
        try:
            ...
            print("(serial closed after reboot: %s)" % e)
            break
        if response:
            print(str(response))
            last_data_at = time.time()
        if not y_sent and response and b'reboot' in response:
            ser.write(b'y\r\n')
            ser.flush()
            y_sent = True
            y_sent_at = time.time()
            continue
        if y_sent and response and b'INIT@STAT?' in response:
            print("(WE-2 application ready, exiting xmodem_send.py)")
            break
        if y_sent and not response and (time.time() - last_data_at) > POST_Y_IDLE_S:
            print("(no further output for %ds after reboot, exiting)" % int(POST_Y_IDLE_S))
            break
